...
description: "Develop eBPF-based network functions and achieve kernel-level performance. eBPF and CNF Development"
---
...
1. **ADR**: Discuss advantages and limitations of using C for kernel-space program and Go for user-space application.
1. **Write an eBPF Program using Restricted C (Kernel Space)**
1. **Write an eBPF Application using and Go (User Space)**
1. **Build an eBPF Program**
1. **Deploy an eBPF Program**
1. **Best practices for eBPF application development**
	- Compile Once Run Everywhere (CO-RE)
	- [X-Factor CNFs](https://x.cnf.dev/)
1. **Service Function Chaining**: Discuss how netkit replaces traditional veth/tc datapaths with a BPF-programmable network device, allowing for host-native packet processing speeds.
